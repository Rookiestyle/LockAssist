...
namespace LockAssist
{
    ...
    internal class SoftLock : IMessageFilter
    {
        ...
        private void Init(QuickUnlock qu)
        {
            ...
            //Prevent focusing group view / entry list view
            var c = Tools.GetControl("m_tvGroups");
            if (c != null) c.GotFocus += OnResetFocus;
            c = Tools.GetControl("m_lvEntries");
            if (c != null) c.GotFocus += OnResetFocus;
            //Prepare for windows opened during Softlock
        ...
        private void OnWindowAdded(object sender, GwmWindowEventArgs e)
        {
            ...
            HandleForm(e.Form, m_dHiddenForms[e.Form], false, null);
        }

        private bool m_bFocusResetRunning = false;
        private void OnResetFocus(object sender, EventArgs e)
        {
            if (!m_SoftLocked) return;
            if (m_bFocusResetRunning) return;
            m_bFocusResetRunning = true;
            string buttonName = "LockAssistPlugin_SoftLock_HideButton" + Program.MainForm.GetType().Name + Program.MainForm.Name;
            Button bHide = Tools.GetControl(buttonName) as Button;

            if (bHide != null)
            {
                bHide.BringToFront();
                bHide.Select();
                PluginDebug.AddInfo("Reset focus",
                    "Focus reset to: " + buttonName,
                    "Focused control: " + (sender as Control).Name);
            }
            else
            {
                PluginDebug.AddInfo("Reset focus",
                    "Focus reset to: " + buttonName,
                    "Focused control: " + (sender as Control).Name,
                    "Error: Unhide button not found");
            }
            m_bFocusResetRunning = false;
        }
        ...
        private void HandleForm(Form f, SLFormProperties slfp, bool bVisible, List<string> lMsg)
        {
            foreach (Control c in f.Controls)
            {
                if (bVisible && slfp.Controls.ContainsKey(c.Name))
                    c.Visible = slfp.Controls[c.Name]; //restore original visibility
                else
                {
                    slfp.Controls[c.Name] = c.Visible; //save original visibility
                    c.Visible = bVisible; //set new visibility
                }
                if (lMsg != null) lMsg.Add("Set visibility " + f.GetType().FullName + "." + c.Name + " - " + c.Visible.ToString());
            }
            HandlePanel(bVisible, f);
            if (!f.RightToLeftLayout)
                f.Opacity = bVisible ? slfp.Opacity : 0.1; //0 makes the window not react on mouse input
        }
